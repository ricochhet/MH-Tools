...
int simple_ini_parser::read(std::string path) {
    ...
    while (std::getline(inifile, line)) {
        ...
        if (std::regex_match(line, std::regex("[[:print:]]+=[[:print:]]+"))) {
            _ini_values.emplace(current_section + std::regex_replace(line, std::regex("=[[:print:]]+"), ""), std::regex_replace(line, std::regex("[[:print:]]+="), ""));
        }
    }
...
bool simple_ini_parser::has(std::string key) {
    if (_ini_values.find(key) != _ini_values.end()) return true;
...
std::string simple_ini_parser::get(std::string key, std::string default_value) {
    // check if key exists
    if (_ini_values.find(key) != _ini_values.end()) return _ini_values.at(key);
...
void simple_ini_parser::print_all(std::ostream& output) {
    for (auto i : _ini_values) output << i.first << "=" << i.second << "\n";
}
