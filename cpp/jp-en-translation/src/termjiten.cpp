...
int main(int argc, char* argv[]) {
    ...
    config.read((getenv("TERMJITEN_CONFIG") == NULL ? ("./termjiten.ini") : getenv("TERMJITEN_CONFIG")));
    ...
    if (option_get_argument("-s", sources, argc, argv)) {
        if (sources.find("jmdict") != std::string::npos) use_jmdict = true;

        if (sources.find("jmnedict") != std::string::npos) use_jmnedict = true;

        if (sources.find("kanjidic") != std::string::npos) use_kanjidic = true;
    ...
    if (use_jmdict) {
        ...
        std::string jmdict_path = config.get("jmdict.path", "./sources/JMdict");
    ...
    if (use_jmnedict) {
        ...
        std::string jmnedict_path = config.get("jmnedict.path", "./sources/JMnedict.xml");
    ...
    if (use_kanjidic) {
        ...
        std::string kanjidic_path = config.get("kanjidic.path", "./sources/kanjidic2.xml");
