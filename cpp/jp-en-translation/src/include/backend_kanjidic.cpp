...
void dictionary_kanjidic::print_entry(pugi::xml_node& character, std::ostream& output, std::map<std::string, std::string>& options) {
    ...
    for (pugi::xml_node reading_meaning : character.children("reading_meaning")) {
        ...
        for (pugi::xml_node rmgroup : reading_meaning.children("rmgroup")) {
            ...
            for (pugi::xml_node reading : rmgroup.children("reading")) {
                // if japanese reading
                if (strcmp(reading.attribute("r_type").as_string(), "ja_on") == 0 || strcmp(reading.attribute("r_type").as_string(), "ja_kun") == 0) {
    ...
    if (verbose) {
        ...
        for (pugi::xml_node rad_name : character.child("misc").children("rad_name")) output << "Radical name: " << rad_name.child_value() << "\n";
        ...
        // grade
        for (pugi::xml_node grade : character.child("misc").children("grade")) output << "Grade (1-10): " << grade.child_value() << "\n";

        // jlpt level
        for (pugi::xml_node jlpt : character.child("misc").children("jlpt")) output << "Former JLPT level (4-1): " << jlpt.child_value() << "\n";
        ...
        for (pugi::xml_node cp_value : character.child("codepoint").children("cp_value")) {
            // Unicode
            if (strcmp(cp_value.attribute("cp_type").as_string(), "ucs") == 0) output << "U+" << cp_value.child_value();

            // JIS X 0208-1997
            if (strcmp(cp_value.attribute("cp_type").as_string(), "jis208") == 0) output << ", JIS X 0208-1997: " << cp_value.child_value();

            // JIS X 0212-1990
            if (strcmp(cp_value.attribute("cp_type").as_string(), "jis212") == 0) output << ", JIS X 0212-1990: " << cp_value.child_value();

            // JIS X 0213-2000
            if (strcmp(cp_value.attribute("cp_type").as_string(), "jis213") == 0) output << ", JIS X 0213-2000: " << cp_value.child_value();
        }
        output << "\n";
...
void dictionary_kanjidic::search(std::string query, std::string method, std::ostream& output, std::map<std::string, std::string>& options) {
    ...
    for (pugi::xml_node character : kanjidic.child("kanjidic2").children("character")) {
        ...
        for (pugi::xml_node literal : character.children("literal")) {
            if (comp(literal.child_value(), query)) match_found = true;
        }
        ...
        for (pugi::xml_node reading : character.child("reading_meaning").child("rmgroup").children("reading")) {
            ...
            if (comp(reading.child_value(), query)) match_found = true;
        }

        // match in nanori
        for (pugi::xml_node nanori : character.child("reading_meaning").children("nanori")) {
            if (comp(nanori.child_value(), query)) match_found = true;
        }

        // match in query code
        for (pugi::xml_node q_code : character.child("query_code").children("q_code")) {
            if (comp(q_code.child_value(), query)) match_found = true;
        }
        ...
        for (pugi::xml_node meaning : character.child("reading_meaning").child("rmgroup").children("meaning")) {
            ...
            if (options["kanjidic.languages"].find(meaning.attribute("m_lang").as_string()) == std::string::npos) continue;

            if (comp(meaning.child_value(), query)) match_found = true;
        }

        // match in codepoint
        for (pugi::xml_node cp_value : character.child("codepoint").children("cp_value")) {
            if (comp(cp_value.child_value(), query)) match_found = true;
        }

        // TODO! search in dic_number.dic_ref

        // if match, print entry
        if (match_found) print_entry(character, output, options);
    }
}
